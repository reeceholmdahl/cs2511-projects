...
public class PuzzleState extends State {
    ...
    public int hashCode() {
        int hash = 7;
        hash = 17 * hash + Arrays.deepHashCode(this.tiles);
        return hash;
    }
...
//        return tilesOutOfPlace(this, goal);
     ...
     */
    public static int tilesOutOfPlace(State current, State _goal) {
        int count = 0;
	PuzzleState curr = (PuzzleState) current;
        PuzzleState goal = (PuzzleState) _goal;
        
        for (int r = 0; r < 3; ++r) {
            for (int c = 0; c < 3; ++c) {
                
                int currTile = curr.tiles[r][c];
                int goalTile = goal.tiles[r][c];
                
                if (currTile != goalTile && goalTile != 0) ++count;
            }
        }
     ...
     */
    public static int sumManhattan(State current, State _goal) {
        int sum = 0;
        
        HashMap<Integer, ArrayList<Integer>> currPos = new HashMap<>();
        
        PuzzleState curr = (PuzzleState) current;
        PuzzleState goal = (PuzzleState) _goal;
        
        for (int r = 0; r < curr.tiles.length; ++r) {
            for (int c = 0; c < curr.tiles[0].length; ++c) {
        ...
        for (int r = 0; r < curr.tiles.length; ++r) {
            for (int c = 0; c < curr.tiles[0].length; ++c) {
                ...
                if (goalTile == 0) continue;

                ArrayList<Integer> position = currPos.get(goalTile);

                int distance = Math.abs(position.get(0) - r) + Math.abs(position.get(1) - c);
                sum += distance;
            }
        }
